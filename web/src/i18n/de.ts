import type { TranslationKey } from './en';

export const de: Record<TranslationKey, string> = {
  appName: 'ausgegeben',
  navRecord: 'erfassen',
  navInsights: 'einblicke',
  navSettings: 'einstellungen',
  navAdd: 'transaktion hinzufügen',
  navMain: 'hauptnavigation',
  screenRecord: 'erfassen',
  screenInsights: 'einblicke',
  screenSettings: 'einstellungen',
  analysisPeriod: 'analysezeitraum',
  noDataForPeriod: 'keine daten für diesen zeitraum',
  currentCategories: 'aktuelle kategorien',
  categoriesEmptyHint: 'noch keine kategorien — füge eine hinzu, um zu starten',
  categoriesEmptyForType: 'noch keine kategorien für diesen typ',
  actionDismiss: 'schließen',
  recordDeleteTitle: 'transaktion löschen',
  recordRowActions: 'transaktionsaktionen',
  descIncomeRow: '{category} Einnahme von {amount}. {note}',
  descExpenseRow: '{category} Ausgabe von {amount}. {note}',
  descTransferRow: '{category} Transfer von {amount}. {note}',
  onboardingStepsLabel: 'einführungsschritte',
  onboardingStepN: 'schritt {n}',
  themeSystem: 'system',
  themeLight: 'hell',
  themeDark: 'dunkel',
  themeAmoled: 'amoled',
  themeMidnight: 'mitternacht',
  themeOcean: 'ozean',
  themeForest: 'wald',
  themeSunset: 'sonnenuntergang',
  themeLavender: 'lavendel',
  themeSoftLight: 'sanftes licht',
  recordEmptyTitle: 'noch keine transaktionen',
  recordEmptySubtitle: 'tippe auf +, um deine erste ausgabe oder einnahme zu erfassen',
  recordEmptyHint: 'tippen zum bearbeiten · lang drücken zum duplizieren · löschen in der zeile',
  recordNoMatchesTitle: 'keine treffer',
  recordNoMatchesSubtitle: 'andere suche versuchen oder filter zurücksetzen',
  recordClearFilters: 'filter zurücksetzen',
  recordSearchPlaceholder: 'notizen oder beträge suchen',
  recordSearchClear: 'suche löschen',
  recordDuplicate: 'duplizieren',
  recordPeriodThisMonth: 'dieser monat',
  recordPeriodAllTime: 'gesamt',
  recordMostSpentOn: 'am meisten ausgegeben für {name}',
  recordSwipeDelete: 'löschen',
  editOrTap: 'bearbeiten',
  recordDeleted: 'transaktion gelöscht',
  recordDuplicated: 'für heute dupliziert',
  recordDeleteConfirm: 'diese Transaktion löschen?',
  transactionSaved: 'transaktion gespeichert',
  transactionUpdated: 'transaktion aktualisiert',
  errorDeleteFailed: 'Transaktion konnte nicht gelöscht werden. Bitte versuche es erneut.',
  errorDuplicateFailed: 'Transaktion konnte nicht dupliziert werden.',
  categoryDeleteLinked: 'Diese Kategorie hat {count} verknüpfte Transaktion(en). Beim Löschen werden sie nach „Unkategorisiert" verschoben. Trotzdem löschen?',
  categoryDeduplicateTitle: 'Doppelte Kategorien zusammenführen?',
  categoryDeduplicateMessage: 'Kategorien mit identischen Namen werden zusammengeführt und ihre Transaktionen kombiniert.',
  categoryDeduplicateConfirm: 'Zusammenführen',
  categoryDeduplicateDone: 'kategorien zusammengeführt',
  categoryErrorAddFailed: 'kategorie konnte nicht hinzugefügt werden',
  categoryErrorUpdateFailed: 'kategorie konnte nicht aktualisiert werden',
  categoryErrorDeleteFailed: 'kategorie konnte nicht gelöscht werden',
  categoryErrorDeduplicateFailed: 'zusammenführen fehlgeschlagen',
  categoryErrorUnwritable: 'neu sortieren nicht möglich: kategorie zuerst korrigieren oder umbenennen — {names}',
  recordUnknownCategory: 'unbekannt',
  filterAll: 'alle',
  filterExpense: 'ausgabe',
  filterIncome: 'einnahme',
  filterTransfer: 'transfer',
  recordTypeFilter: 'transaktionstyp',
  summaryEarned: 'einnahmen',
  summarySpent: 'ausgaben',
  summaryBalance: 'saldo',
  budgetMonthlyLabel: 'monatsbudget',
  budgetProgress: '{spent} / {budget}',
  budgetOverBy: '{amount} über budget',
  billsEmptyTitle: 'noch keine analysen',
  billsEmptySubtitle: 'erfasse ausgaben, einnahmen oder transfers für auswertungen',
  billsTotalTransfers: 'gesamttransfers',
  settingsTheme: 'design',
  settingsCurrency: 'währung',
  currencyEur: 'Euro (€)',
  currencyUsd: 'US-Dollar ($)',
  currencyGbp: 'Pfund Sterling (£)',
  currencyChf: 'Schweizer Franken (CHF)',
  settingsLanguage: 'sprache',
  settingsCategories: 'kategorien verwalten',
  settingsExport: 'als csv exportieren',
  settingsOffline: 'nicht angemeldet',
  settingsOfflineSub: 'anmeldung erforderlich, um ausgegeben zu nutzen',
  settingsSignIn: 'anmelden / konto erstellen',
  settingsMonthlyLimit: 'monatliches ausgabenlimit',
  settingsMonthlyLimitNotSet: 'nicht festgelegt',
  settingsBudgetSet: 'monatslimit gesetzt',
  settingsBudgetCleared: 'monatslimit entfernt',
  authTagline: 'dein geld, deine regeln',
  authSubtitle: 'melde dich an, um ausgaben und kategorien auf all deinen geräten zu synchronisieren.',
  authSubtitleCloud: 'melde dich an, um mit der android-app zu synchronisieren.',
  authCloudHint: 'verwende dieselbe e-mail und dasselbe passwort wie auf dem handy.',
  authSignIn: 'anmelden',
  authSignUp: 'registrieren',
  authEmail: 'e-mail',
  authPassword: 'passwort',
  authConfirmPassword: 'passwort bestätigen',
  authForgotPassword: 'passwort vergessen?',
  authRememberMe: 'angemeldet bleiben',
  authRememberMeSub: 'auf geteilten oder öffentlichen Geräten deaktiviert lassen',
  authResetEmailSent: 'passwort-reset-e-mail gesendet. prüfe deinen posteingang.',
  authErrorEmailRequired: 'gib deine e-mail-adresse ein.',
  authErrorPasswordMismatch: 'passwörter stimmen nicht überein.',
  authOfflineHint: 'transaktionen synchronisieren über die cloud.',
  authFirebaseNotConfigured: 'vite_firebase_* variablen hinzufügen, um cloud-sync zu aktivieren.',
  authErrorUnauthorizedDomain: 'diese website ist in firebase nicht autorisiert. domain unter authentication → einstellungen → autorisierte domains hinzufügen.',
  authErrorInvalid: 'ungültige e-mail oder passwort.',
  authErrorEmailInUse: 'diese e-mail ist bereits registriert.',
  authErrorWeakPassword: 'passwort muss mindestens 8 zeichen haben.',
  authErrorTooManyAttempts: 'zu viele versuche. versuche es später erneut.',
  authErrorNetwork: 'netzwerkfehler. prüfe deine verbindung und versuche es erneut.',
  authErrorAppCheck: 'sicherheitsprüfung fehlgeschlagen. lade die seite neu und versuche es erneut.',
  authShowPassword: 'passwort anzeigen',
  authHidePassword: 'passwort verbergen',
  authErrorGeneric: 'anmeldung fehlgeschlagen. bitte erneut versuchen.',
  authVerifyEmailSent: 'bestätigungsmail gesendet. bitte posteingang prüfen, um registrierung abzuschließen.',
  authVerifyBanner: 'bestätige deine e-mail, damit konto-wiederherstellung verfügbar bleibt.',
  authVerifyRequired: 'bestätige deine e-mail, um transaktionen zu speichern.',
  authVerifyResend: 'e-mail erneut senden',
  authVerifyRefresh: 'ich habe bestätigt',
  authVerifyDismiss: 'schließen',
  actionBack: 'zurück',
  syncInProgress: 'daten werden synchronisiert…',
  syncLastAt: 'zuletzt synchronisiert {time}',
  syncNever: 'noch nicht synchronisiert',
  syncNow: 'jetzt synchronisieren',
  syncOk: 'synchronisiert — {expenses} transaktionen, {categories} kategorien aktualisiert',
  settingsCloudAccount: 'cloud-konto',
  settingsAccountSyncEnabled: 'cloud-backup aktiv',
  settingsSignedInAs: 'angemeldet als {email}',
  settingsSignOut: 'abmelden',
  settingsLocalCleanupFailed: 'abgemeldet, aber lokale Finanzdaten konnten nicht gelöscht werden. Schließe andere App-Tabs und lösche die Websitedaten.',
  settingsSignOutConfirm: 'abmelden? zum erneuten zugriff musst du dich anmelden.',
  settingsSyncErrorPermission: 'cloud-sync konnte nicht auf dein konto zugreifen. melde dich ab und wieder an.',
  settingsSyncErrorNetwork: 'keine internetverbindung. prüfe dein netzwerk und versuche es erneut.',
  settingsSyncErrorGeneric: 'cloud-sync ist vorübergehend nicht verfügbar. bitte erneut versuchen.',
  settingsSyncRetry: 'sync erneut versuchen',
  settingsDeleteAccount: 'konto löschen',
  settingsDeleteAccountConfirm: 'konto sowie alle ausgaben, kategorien und einstellungen dauerhaft löschen? das kann nicht rückgängig gemacht werden.',
  settingsDeleteAccountFailed: 'konto konnte nicht gelöscht werden',
  settingsDeleteAccountNeedsReauth: 'melde dich erneut an und lösche dann dein konto',
  settingsDeleteAccountPassword: 'gib zum bestätigen dein passwort ein',
  settingsDeleteAccountTooManyAttempts: 'zu viele versuche. versuche es später erneut.',
  settingsDeleteAccountOk: 'konto gelöscht',
  settingsDeleteAccountIncomplete:
    'cloud-daten wurden gelöscht, aber die anmeldung konnte nicht entfernt werden. tippe erneut auf konto löschen, um abzuschließen.',
  settingsDeletionPending:
    'die löschung dieses kontos ist unvollständig. die cloud-daten sind bereits gelöscht, und die löschung kann nicht abgebrochen werden — schließe sie ab, um auch die anmeldung zu entfernen.',
  settingsDeletionFinish: 'löschung abschließen',
  settingsVersionSubtitle: 'version {version}',
  settingsVersion: 'version',
  settingsPreferences: 'einstellungen',
  settingsData: 'daten',
  settingsAbout: 'über',
  settingsErrorReporting: 'absturzberichte senden',
  settingsErrorReportingSub: 'wenn aktiv, gehen unbehandelte fehler (meldung, stack, seitenpfad, browser) an einen eigenen endpunkt — dein konto wird nie mitgeschickt, bekannte geheimnisse werden vorher entfernt',
  onboardingGetStarted: 'loslegen',
  onboardingNext: 'weiter',
  onboardingSkip: 'überspringen',
  actionSave: 'speichern',
  actionSaving: 'wird gespeichert…',
  actionCancel: 'abbrechen',
  actionClear: 'löschen',
  actionClose: 'schließen',
  actionSkipToContent: 'zum inhalt springen',
  actionUndo: 'rückgängig',
  addUnsavedTitle: 'ungespeicherte änderungen',
  addUnsavedBody: 'du hast ungespeicherte änderungen. verwerfen?',
  addDiscard: 'verwerfen',
  addTransaction: 'transaktion hinzufügen',
  editTransaction: 'transaktion bearbeiten',
  addAmountLabel: 'betrag',
  addCategoryLabel: 'kategorie',
  noteLabel: 'notiz / händler',
  notePlaceholder: 'optionale notiz',
  budgetPlaceholder: '0,00',
  authEmailPlaceholder: 'name@beispiel.de',
  categoryDeleteMessage: 'Dies kann nicht rückgängig gemacht werden. Verknüpfte Transaktionen werden nach „Unkategorisiert“ verschoben.',
  dateLabel: 'datum',
  periodAllTime: 'gesamt',
  periodPickerLabel: 'zeitraum',
  periodPickerTitle: 'zeitraum wählen',
  periodPickerSubtitle: 'einblicke und diagramme für einen monat oder die gesamte historie anzeigen.',
  periodPickerMonths: 'nach monat',
  loading: 'laden…',
  errorTitle: 'etwas ist schiefgelaufen',
  errorMessage: 'die app hat einen unerwarteten fehler festgestellt. neu laden behebt das meistens.',
  errorReload: 'neu laden',
  errorHardReset: 'ohne cache neu laden',
  errorDetails: 'fehlerdetails',
  langEnglish: 'englisch',
  langGerman: 'deutsch',
  errorValidAmount: 'gültigen betrag eingeben',
  errorChooseCategory: 'kategorie wählen',
  budgetInvalidHint: 'betrag größer als null eingeben',
  errorLoadFailed: 'laden fehlgeschlagen. bitte erneut versuchen.',
  errorLoadFailedHint: 'verbindung prüfen und erneut versuchen',
  errorBudgetExceeded: 'monatsbudget überschritten: {spent} von {budget}',
  errorBudgetCheckFailed: 'monatsbudget konnte nicht geprüft werden. die transaktion wurde gespeichert.',
  errorSaveFailed: 'speichern fehlgeschlagen. bitte erneut versuchen.',
  actionRetry: 'erneut versuchen',
  typeExpense: 'ausgabe',
  typeIncome: 'einnahme',
  typeTransfer: 'transfer',
  chartTotal: 'gesamt',
  chartCategoryBreakdown: 'kategorieverteilung',
  chartCashFlow: 'cashflow-diagramm',
  timeToday: 'heute',
  timeYesterday: 'gestern',
  transactionDefault: 'transaktion',
  billsNet: 'saldo',
  billsCashFlow: 'cashflow',
  billsCashFlowSubtitle: 'einnahmen {income} · ausgaben {expense}',
  billsCashFlowHint: 'summen in {currency} — genaue beträge in der liste.',
  addCategory: 'kategorie hinzufügen',
  categoryNamePrompt: 'kategoriename',
  categoryEditTitle: 'kategorie bearbeiten',
  categoryMoveUp: 'nach oben verschieben',
  categoryMoveDown: 'nach unten verschieben',
  categoryInvalidName: 'gültigen kategorienamen eingeben',
  categoryIconLabel: 'symbol',
  categoryColorLabel: 'farbe',
  categoryOther: 'sonstige',
  categoryDeleteConfirm: '„{name}“ und verknüpfte transaktionen löschen?',
  actionDelete: 'löschen',
  settingsBudget: 'budget',
  settingsManagement: 'verwaltung',
  settingsCategoriesSub: 'kategorien hinzufügen oder entfernen',
  settingsTrustedDevice: 'Offline-Speicher für vertrauenswürdiges Gerät',
  settingsTrustedDeviceSub: 'Finanzdaten über Browser-Neustarts behalten; auf gemeinsam genutzten Geräten deaktiviert lassen',
  settingsPersistentAuth: 'angemeldet bleiben',
  settingsPersistentAuthSub: 'über Browser-Neustarts hinweg angemeldet bleiben; auf geteilten Geräten deaktiviert lassen',
  settingsExportSub: 'tabelle herunterladen',
  settingsExportOk: 'export bereit',
  settingsExportTruncated: 'export bereit (neueste 5000 zeilen)',
  settingsExportTruncatedConfirm:
    'nur die neuesten 5000 buchungen werden enthalten. ältere zeilen fehlen. fortfahren?',
  settingsExportTruncatedContinue: 'trotzdem exportieren',
  dataTruncatedNotice: 'zeige die neuesten 5.000 transaktionen — ältere einträge sind ausgeblendet',
  settingsExportFailed: 'export fehlgeschlagen',
  settingsRemindersPhoneOnly: 'tägliche erinnerungen werden in der android-app verwaltet',
  settingsChooseTheme: 'design wählen',
  settingsChooseCurrency: 'währung wählen',
  onboardingPage1Title: 'jede ausgabe erfassen',
  onboardingPage1Body: 'erfasse ausgaben, einnahmen und überweisungen in sekunden mit dem numpad.',
  onboardingPage2Title: 'sieh, wohin das geld fließt',
  onboardingPage2Body: 'donut-diagramme und cashflow-trends machen deine historie verständlich.',
  onboardingPage3Title: 'bleib im budget',
  onboardingPage3Body: 'setze ein monatslimit und verfolge deinen fortschritt auf dem erfassen-tab.',
  onboardingPage4Title: 'überall synchron',
  onboardingPage4Body: 'melde dich an, um deine daten per firebase mit der android-app abzugleichen.',
  catGroceries: 'Lebensmittel',
  catShopping: 'Shopping',
  catDining: 'Restaurant',
  catTransport: 'Transport',
  catBills: 'Rechnungen',
  catSubscriptions: 'Abos',
  catSalary: 'Gehalt',
  catFreelance: 'Freelance',
  catRefunds: 'Rückerstattung',
  catTransfer: 'Transfer',
  iconCategory: 'kategorie',
  iconShoppingCart: 'einkaufswagen',
  iconShoppingBag: 'einkaufstasche',
  iconRestaurant: 'restaurant',
  iconCafe: 'café',
  iconCar: 'auto',
  iconGas: 'kraftstoff',
  iconHome: 'zuhause',
  iconBolt: 'strom',
  iconWifi: 'internet',
  iconSubscriptions: 'abos',
  iconSmoking: 'rauchen',
  iconHealth: 'gesundheit',
  iconFitness: 'fitness',
  iconEducation: 'bildung',
  iconWork: 'arbeit',
  iconFlight: 'flug',
  iconHotel: 'hotel',
  iconWallet: 'geldbörse',
  iconSavings: 'sparen',
  iconCreditCard: 'kreditkarte',
  iconTrendingUp: 'anlagen',
  iconUndo: 'rückerstattung',
  iconSwap: 'transfer',
  iconGift: 'geschenk',
  iconEntertainment: 'unterhaltung',
  iconPets: 'haustiere',
  iconChild: 'kind',
  iconPhone: 'telefon',
  iconLaptop: 'laptop',
  iconAwards: 'preise',
  iconHelp: 'hilfe',
};
